...
def plot_spikes(population, title, timeStepMs):
    ...
    for i, n in enumerate(population):
        xs = map(lambda (i, s): i * timeStepMs,
                 filter(lambda (i, s): s, enumerate(population[i])))
...
def load_spike_recording(spikeFilename, numVectorWords):
    with open(spikeFilename, mode='rb') as spikeFile:
        ...
        print(
        "Spike file contains %u bytes of spike data = %u simulation ticks" % (
        ...
        for tick in range(0, numberOfTimeStepsWritten):
            ...
            for neuronWordIndex in range(0, numVectorBytes, 4):
                ...
                spikeVectorWord = struct.unpack_from("<I", spikeFileContents,
                ...
                for neuronBitIndex in range(0, 32):
                    ...
                    neurons.append(1 if (
                    (spikeVectorWord[0] & neuronBitMask) != 0) else 0)
...
if __name__ == "__main__":
    ...
    parser.add_option("-n", "--num-vector-words", dest="numVectorWords",
                      action="store", type="int", default=8,
                      help="number of words that are used to represent a spike vector")
    (options, args) = parser.parse_args()

    if options.filename == None:
        parser.error("No filename specified")
    else:
        populations = load_spike_recording(options.filename,
                                           options.numVectorWords)
