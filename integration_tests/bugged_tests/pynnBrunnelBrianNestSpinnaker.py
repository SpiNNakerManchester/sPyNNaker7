...
if esp is not None:
    ...
    pblt._make_plot(ts_ext, ts_ext, ids_ext, ids_ext, True, 5.0, False,
...
if isp is not None:
    ...
    pblt._make_plot(ts_inh, ts_inh, ids_inh, ids_inh, False, 5.0, False,
                    'Raster Plot of the inhibitory population in %s' % simulator_Name,
                    'Simulation Time (ms)', total_time=sim_time, n_neurons=N_I)

if pe is not None:
    ts_pe = [x[1] for x in pe]
    ids_pe = [x[0] for x in pe]
    pblt._make_plot(ts_pe, ts_pe, ids_pe, ids_pe, True, 5.0, False,
                    'Raster Plot of the source excitatory population in %s' % simulator_Name,
                    'Simulation Time (ms)', total_time=sim_time, n_neurons=N_E)
