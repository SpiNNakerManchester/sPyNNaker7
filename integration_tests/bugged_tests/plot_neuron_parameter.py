...
def load_neuron_parameter_recording(parameterFilename, numNeurons, timeStepMs):
    with open(parameterFilename, mode='rb') as parameterFile:
        ...
        numberOfBytesWritten = struct.unpack_from("<I", parameterFileContents)[
            0]
        numberOfTimeStepsWritten = numberOfBytesWritten / numVectorBytes
        print(
        "Neuron parameter file contains %u bytes of spike data = %u simulation ticks" % (
        numberOfBytesWritten, numberOfTimeStepsWritten))

        parameterTimes = range(0, numberOfTimeStepsWritten * timeStepMs,
                               timeStepMs)
        ...
        for neuronID in range(numNeurons):
            ...
            for tick in range(0, numberOfTimeStepsWritten):
                ...
                neuronParameterOffset = 4 + (tick * numVectorBytes) + (
                neuronID * 4)

                # Unpack and scale parameter word
                neuronParameterValue = float(
                    struct.unpack_from("<i", parameterFileContents,
                                       neuronParameterOffset)[0]) / scale
...
if __name__ == "__main__":
    ...
    parser.add_option("-f", "--file", dest="filename", action="store",
                      type="string",
                      help="file containing data written to memory by SpiNNaker")
    parser.add_option("-t", "--timestep", dest="timestep", action="store",
                      type="int", default=1, help="simulation timestep in ms")
    parser.add_option("-n", "--num-neurons", dest="numNeurons", action="store",
                      type="int", help="number of neurons")
    parser.add_option("-i", "--neuron-id", dest="neuronID", action="store",
                      type="int", help="id of neuron's parameter to display")
    ...
    if options.filename == None:
        ...
        parameters_t, parameters_value = load_neuron_parameter_recording(
            options.filename, options.numNeurons, options.timestep)
