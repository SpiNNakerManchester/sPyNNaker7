...
for i_mini in range(n_attr):
    exc_populations.append(
        p.Population(n_exc_hyper / n_attr, neuron_model, cell_params,
                     label="pop %u" % i_mini))
...
for exc_pop in enumerate(exc_populations):
    noise_gen = p.Population(n_exc_hyper / n_attr, p.SpikeSourcePoisson,
                             {'rate': noise_rate, 'duration': simtime,
                              'start': 0}, label="noise %u" % exc_pop[0])
    p.Projection(noise_gen, exc_pop[1],
                 p.OneToOneConnector(weights=0.1, delays=1),
                 target='excitatory')
...
for cell in enumerate(exc_populations):
    ...
    if spikes != None:
        pylab.plot([i[1] for i in spikes],
                   [i[0] + (n_exc_hyper / n_attr * cell[0]) for i in spikes],
                   ".")
    else:
