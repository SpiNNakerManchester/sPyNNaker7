...
activity_colours = ['#800000', '#FF0000', '#FF8000', '#FFC000', '#FFE000',
                    '#FFFF00', '#FFFF80', '#FFFFC0', '#FFFFE0', '#FFFFF0',
                    '#FFFFF8', '#FFFFFC', '#FFFFFE', '#FFFFFF']
direction_colours = ['#400000', '#800000', '#C00000', '#FF0000', '#004000',
                     '#008000', '#00C000', '#00FF00', '#000040', '#000080',
                     '#0000C0', '#0000FF', '#404000', '#808000', '#C0C000',
                     '#FFFF00', '#400040', '#800080', '#C000C0', '#FF00FF',
                     '#004040', '#008080', '#00C0C0', '#00FFFF', '#402000',
                     '#804000', '#C06000', '#FF8000', '#400020', '#800040',
                     '#C00060', '#FF0080']
colour_map_activity = colours.ListedColormap(colors=activity_colours,
                                             name='activity_intensity')
colour_map_direction = colours.ListedColormap(colors=direction_colours,
                                              name='direction_strength')


def mapped_boxplot(data, x_dim, y_dim, t_max, tau=1, linewidth=1,
                   screen_size=24, screen_aspect=(16, 10),
                   screen_v_resolution=1200, max_v_screen_occupancy=0.8):
    """ 
    ...
    """
    dpi = int(float(screen_v_resolution) / (math.sqrt(
        float(screen_size ** 2) / float(
            screen_aspect[0] ** 2 + screen_aspect[1] ** 2)) * screen_aspect[
                                                1]))  # compute how many effective dots per inch the screen in use has. Needed for matplotlib scaling

    # first part: massage the data into a convenient sorted form
    sort_ids = numpy.lexsort((data[:, 0], data[:,
                                          1]))  # sort the data by neuron first, then by time of spike.
    map_data = numpy.array(
        [[int(data[row, 1]) / y_dim, int(data[row, 1]) % y_dim, data[row, 0]]
         for row in sort_ids])
    ...
    if (
                2 * linewidth * t_max * y_dim) > screen_v_resolution * max_v_screen_occupancy:
        # adjust for thick linewidths that would occupy more than the entire screen when combined
        linewidth = math.floor(
            screen_v_resolution * max_v_screen_occupancy) / float(
            2 * t_max * y_dim)
        print "Warning: requested linewidth wider than maximum allowable."
        print "Setting linewidth to maximum possible: %f" % linewidth
    plot_size = [2 * linewidth * t_max * x_dim,
                 2 * linewidth * t_max * y_dim]  # total size of the topographic plot
    sizes = [(2 * linewidth * (t_max - row[2] + 1)) ** 2 for row in
             map_data]  # sizes of the possible boxes
    colour_range = colours.Normalize(vmin=0.0, vmax=14.0,
                                     clip=True)  # map the activity range to the possible colours
    integr_idx = 0.0
    t_prev = map_data[0, 2]
    neuron = map_data[0, :2]
    attn_colours = []
    ...
    for row in map_data:  # now go through each neuron in the data. row will be an array of times.
        same_neuron = (row[
                       :2] == neuron)  # VERY tricky: an array comparison actually returns an array
        if all(same_neuron):  # this means the neuron indices matched
            integr_idx = integr_idx - ((row[
                                            2] - t_prev) / tau)  # update the leaky integrator to the current time
            attn_colours.append(
                integr_idx)  # decide what activity colour it should be
            integr_idx += 1.0  # each spike raises the activity by 1
            t_prev = row[2]  # store the spike time
        else:  # new neuron
            attn_colours.append(
                0.0)  # first colour must be the lowest
            integr_idx = 1.0  # and there's only one spike so far in the integrator
            t_prev = row[2]  # store the spike time
            neuron = row[
                     :2]  # use this neuron as the reference to compare when checking for a new neuron

    # fourth part: output the data in text and plot format
    data_out = open("boxplt_dat.txt", "w")
    data_out.write(
        "sorted spikes: %s" % map_data)  # output the spikes to a file for any desired off-line analysis
    data_out.close()
    # the line below actually creates the scatter plot   
    box_plot = plt.scatter(x=map_data[:, 0], y=map_data[:, 1], s=sizes,
                           c=attn_colours, marker='s',
                           cmap=colour_map_activity, norm=colour_range, vmin=0,
                           vmax=14, linewidths=(linewidth,), facecolors='none')
    ...
    focus_plot.set_size_inches(plot_size[0] / dpi, plot_size[1] / dpi,
                               forward=True)  # and plot size
    ...
    focus_plot_axes.set_autoscale_on(
        False)  # we have manually scaled so don't autoscale
    focus_plot_axes.set_frame_on(False)  # no inner background either
    focus_plot_axes.set_xlim(left=0,
                             right=x_dim)  # set plot limits to topographic area
    focus_plot_axes.set_ylim(bottom=0, top=y_dim)
    return box_plot


def mapped_arrowplot(data, x_dim, y_dim, t_max, tau=1, arrowscale=1.0,
                     linewidth=1, screen_size=24, screen_aspect=(16, 10),
                     screen_v_resolution=1200, max_v_screen_occupancy=1.0):
    """ 
    ...
    dpi = int(float(screen_v_resolution) / (math.sqrt(
        float(screen_size ** 2) / float(
            screen_aspect[0] ** 2 + screen_aspect[1] ** 2)) * screen_aspect[
                                                1]))  # same dpi computation as for mapped_boxplot
    ...
    sort_ids = [
        numpy.lexsort((data[dir_idx][:, 0], data[dir_idx][:, 1])) if len(
            data[dir_idx]) > 0 else numpy.array([]) for dir_idx in dir_idxs]
    map_data = [numpy.array([[int(data[dir_idx][row, 1]) / y_dim,
                              int(data[dir_idx][row, 1]) % y_dim,
                              data[dir_idx][row, 0]] for row in
                             sort_ids[dir_idx][:]]) for dir_idx in dir_idxs]
    ...
    plot_prescale = screen_v_resolution * max_v_screen_occupancy / plot_size[
        1]  # scaling factor for the plot
    plot_prescale = math.floor(
        plot_prescale) if plot_prescale > 1.0 else plot_prescale
    ...
    angles_u = reduce(lambda x, y: x + y, [
        [math.cos((math.pi / num_dirs) * dir_idx)] * len(map_data[dir_idx]) for
        dir_idx in dir_idxs if len(map_data[dir_idx]) > 0], [])
    angles_v = reduce(lambda x, y: x + y, [
        [math.sin((math.pi / num_dirs) * dir_idx)] * len(map_data[dir_idx]) for
        dir_idx in dir_idxs if len(map_data[dir_idx]) > 0], [])
    ...
    # context.
    positions_x = reduce(lambda x, y: x + y, [
        [(0.5 + (int(col[2]) / t_dim)) + (col[0] * cell_dim) for col in
         map_data[dir_idx]] for dir_idx in dir_idxs if
        len(map_data[dir_idx]) > 0], [])
    positions_y = reduce(lambda x, y: x + y, [
        [(0.5 + (int(col[2]) % t_dim)) + (col[1] * cell_dim) for col in
         map_data[dir_idx]] for dir_idx in dir_idxs if
        len(map_data[dir_idx]) > 0], [])
    ...
    colour_range = colours.BoundaryNorm(
        boundaries=range(len(direction_colours)),
        ncolors=len(direction_colours), clip=True)
    integr_idx = numpy.zeros((num_dirs,), float)
    t_prev = [
        numpy.array(map_data_vals[0, 2] if len(map_data_vals) > 0 else []) for
        map_data_vals in map_data]
    neuron = [
        numpy.array(map_data_vals[0, :2] if len(map_data_vals) > 0 else []) for
        map_data_vals in map_data]
    dir_colours = [[] for i in
                   dir_idxs]  # set up a colour array for each orientation
    for dir_idx in [idx for idx in dir_idxs if len(map_data[idx]) > 0]:
        for row in map_data[
            dir_idx]:  # map the activity arrays, following the leaky integrator, for a given orientation
            if all((row[:2] == neuron[
                dir_idx])):  # VERY tricky: an array comparison actually returns an array
                integr_idx[dir_idx] = max(
                    integr_idx[dir_idx] - ((row[2] - t_prev[dir_idx]) / tau),
                    3)
                dir_colours[dir_idx].append(
                    (4 * dir_idx) + integr_idx[dir_idx])
    ...
    dir_colours = reduce(lambda x, y: x + y,
                         [dir_cmap for dir_cmap in dir_colours if
                          len(dir_cmap) > 0], [])
    ...
    data_out.write(
        "sorted spikes: %s" % map_data)  # output the spikes to a file for any desired off-line analysis
    data_out.close()
    arrow_plot = plt.quiver(positions_x, positions_y, angles_u, angles_v,
                            dir_colours, units='xy', scale=arrowscale,
                            scale_units='xy',
                            width=linewidth * arrowscale / plot_prescale,
                            pivot='middle', cmap=colour_map_direction,
                            norm=colour_range)
    ...
    dir_plot.set_size_inches(plot_size[0] * plot_prescale / dpi,
                             plot_size[1] * plot_prescale / dpi, forward=True)
    dir_plot.set_frameon(True)  # Background on: black tends to hide arrows
    dir_plot.set_facecolor(
        '#808080')  # for the arrow plot a grey background is easiest to see
