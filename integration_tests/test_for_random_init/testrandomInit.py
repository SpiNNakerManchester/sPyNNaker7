...
# Simulation Setup
setup(timestep=time_step, min_delay=1.0, max_delay=11.0,
      db_name='test_RandomInit.sqlite')
...
for i in range(orientations):  # Cycles orientations
    ...
    v4_pop.append(
        Population(base_v4_num_neurons * base_v4_num_neurons,  # size
    ...
    if i == aversive_orientation:  # aversive orientation connectivity projects to other orientations ADR
        if aversive_inhibitory:
            projections.append(Projection(pfc[i],
                                          v4_pop[i],
                                          AllToAllConnector(
                                              weights=-pfc_v4_weights,
                                              delays=delays),
                                          target='inhibitory'))
        else:
            for j in [orientation for orientation in range(orientations) if
                      orientation != i]:
                projections.append(Projection(pfc[i],
                                              v4_pop[j],
                                              AllToAllConnector(
        ...
        projections.append(Projection(pfc[i],
                                      v4_pop[i],
                                      AllToAllConnector(weights=pfc_v4_weights,
                                                        delays=delays),
                                      target='excitatory'))
...
if layer_to_observe == 'v4':
    ...
    for i in range(len(v4_pop)):
        ...
        if vector_plot:
            ...
            if len(data) > 0:
                plt.scatter(data[:, 0], data[:, 1] + id_accumulator,
                            color='green', s=4)  # s=1
            id_accumulator = id_accumulator + v4_pop[i].size
    if vector_plot:
        mapped_arrowplot(data_vector, x_dim=scale.v4_pop_size[0],
                         y_dim=scale.v4_pop_size[1], t_max=runtime)
    ...
    if vector_plot:
        mapped_arrowplot(data_vector, x_dim=scale.pfc_pop_size[0],
                         y_dim=scale.pfc_pop_size[1], t_max=runtime)
