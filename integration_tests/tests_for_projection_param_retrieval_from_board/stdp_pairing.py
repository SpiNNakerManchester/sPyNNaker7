...
post_stim = p.Population(pop_size, p.SpikeSourceArray, {'spike_times': [
    [i for i in
     range(pairing_start_time, pairing_end_time, time_between_pairs)], ]})
...
stdp_model = p.STDPMechanism(
    ...
    weight_dependence=p.AdditiveWeightDependence(w_min=0, w_max=1, A_plus=0.02,
                                                 A_minus=0.02)
)
