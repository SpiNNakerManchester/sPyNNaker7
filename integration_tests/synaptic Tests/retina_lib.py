...
def Gabor_creation(M, N, longe):
    ...
    for m in range(M):
        for n in range(N):
            ...
            este = tan(pi / (2 * K)) * (
            Uh - 2 * log(2) * ((pow(sigu, 2)) / Uh))
            este2 = pow((2 * log(2) - (
            ((pow((2 * log(2)), 2)) * (pow(sigu, 2))) / (pow(Uh, 2)))),
                        (-1.0 / 2.0))
            temp = (2 * log(2) - (
            ((pow((2 * log(2)), 2)) * (pow(sigu, 2))) / (pow(Uh, 2))))
            sigv = tan(pi / (2 * K)) * (
            Uh - 2 * log(2) * ((pow(sigu, 2)) / Uh)) * pow(temp, (-1.0 / 2.0))
            ...
            for x in range(-int(size1), int(size2) + 1):
                ...
                for y in range(-int(size1), int(size2) + 1):
                    ...
                    phi1 = (-1.0 / 2.0) * (
                    (xb * xb) / (sigmax * sigmax) + (yb * yb) / (
                    sigmay * sigmay))
                    phi = (1.0 / (2 * pi * sigmax * sigmay)) * (
                    (exp(phi1)) * (exp(2j * pi * W * xb)))
...
def Filter1DConnector(size_in1, size_in2, size_out1, size_out2, weights,
    ...
    mylist = Filter2DConnector_jose(size_in1, size_in2, size_out1, size_out2,
                                    weights, size_k1, size_k2, jump, delays,
                                    gain=1)
    out = []
    for i in mylist:
            ...
            for x in range(-size_gabor / 2 + 1, size_gabor / 2 + 1):
                for y in range(-size_gabor / 2 + 1, size_gabor / 2 + 1):
                    if (pre_x + x >= 0) \
                        ...
                        weight = kernel_matrix[
                                     (x + size_gabor / 2) * size_gabor + (
                                     y + size_gabor / 2)] * gain
                        if (precision > 0 and abs(
                                weight) > 1 / 2.0 ** precision) or precision == 0:
...
def subSamplerConnector2D(size_in, size_out, weights, delays):
    ...
    for i in range(size_in):
        for j in range(size_in):
            ...
            j_out = j / step
            out.append(
                (i * size_in + j, i_out * size_out + j_out, weights, delays))
    return out
...
def ProximityConnector(row_size, col_size, receptive_field, weights, delays,
    ...
    for r in range(0, row_size, receptive_field):
        for c in range(0, col_size, receptive_field):
            ...
            connector_list = get_neighbour_ids(
                translate_2D_to_1D(r, c, row_size), row_size, col_size,
                receptive_field)
            out_temp = []
            out_temp = [(i, j, weights, delays) for i in connector_list for j
                        in connector_list]  # create all possible connections

            if allow_self_connections == False:
                out_temp = [o for o in out_temp if
                            o[0] != o[1]]  # will eliminate self connections
            for o in out_temp:
                out.append(o)
    return (out)
    ...
    for r in range(receptive_field):
        for c in range(receptive_field):
            ...
            if (x < col_size) and (y < row_size):
                ...
                if not (
                        dest_id == neuron_id and allow_self_connections == False):
                    out.append(dest_id)
    return out
...
def gaussians(size, orientations, sigma=1):
    ...
    for angle in angles:
        out.append(
            ndimage.rotate(kernel, angle, reshape=False, mode='nearest'))
