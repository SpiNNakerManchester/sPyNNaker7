...
class TestVABenchmarkSpikes(unittest.TestCase):
    ...
    def test_va_benchmark(self):
        ...
        n_exc = int(
            round((n * r_ei / (1 + r_ei))))  # number of excitatory cells
        ...
        w_exc = 1e-3 * g_exc * (
        e_rev_exc - v_mean)  # (nA) weight of excitatory synapses
        ...
        exc_conn = FixedProbabilityConnector(pconn, weights=w_exc,
                                             delays=delay)
        inh_conn = FixedProbabilityConnector(pconn, weights=w_inh,
                                             delays=delay)
