"""
NOTE!!!!!!
needs parameters 14000, 1000, 0.01
"""
...
def analyseTimeSeries(pots, numNeurons, interval):
    ...
    for elem in pots:
        (neurID, timeStamp, potential) = elem
        if (timeStamp > 99):
            vars[neurID] += (potential - means[neurID]) * (
            potential - means[neurID])
...
if len(sys.argv) is not 4:
    print "Error: expected 3 parameters after filename, but got ", len(
        sys.argv) - 1
    quit()
...
E2ImeanDelay = 3.0
I2EmeanDelay = 3.0
E2E_delay_distr = RandomDistribution('normal', parameters=[E2EmeanDelay, 0.75],
                                     rng=rng, boundaries=[1.0, 12.0],
                                     constrain='redraw')
E2I_delay_distr = RandomDistribution('normal', parameters=[E2ImeanDelay, 0.75],
                                     rng=rng, boundaries=[1.0, 10.0],
                                     constrain='redraw')
I2E_delay_distr = RandomDistribution('normal', parameters=[I2EmeanDelay, 0.75],
                                     rng=rng, boundaries=[1.0, 10.0],
                                     constrain='redraw')
...
populations.append(p.Population(nL1ExcitNeurons, p.SpikeSourcePoisson,
                                {'rate': noiseRate, 'duration': runTime},
                                label='L1Excit'))
populations.append(
    p.Population(nL1InhibNeurons, p.IF_curr_exp, cell_params_lif,
                 label='L1Inhib'))
# Layer 2 (hidden layer) populations:
populations.append(
    p.Population(nL2ExcitNeurons, p.IF_curr_exp, cell_params_lif,
                 label='L2Excit'))

## Projections
# Starting in layer 1:
projections.append(p.Projection(populations[L1E], populations[L1I],
                                p.FixedProbabilityConnector(p_connect=pL1E2I,
                                                            weights=L1MeanWeightX2X,
                                                            delays=E2ImeanDelay),
                                target='excitatory'))  # From excit to its own inhib neurons
projections.append(p.Projection(populations[L1E], populations[L2E],
                                p.FixedProbabilityConnector(p_connect=pL1E2E,
                                                            weights=L1MeanWeightX2X,
                                                            delays=E2ImeanDelay),
                                target='excitatory'))  # Feedforward excitation
...
myString = "%d %d %.2f    %.2f %.2f %.2f %.2f" % (
nL1ExcitNeurons, nL2ExcitNeurons, \
