...
if benchmark == "COBA":
    ...
    Ginh = 4.5    # (nS)
    #  but the delpolarising drift should be taken into account
Erev_exc = 0.     # (mV)
Erev_inh = -80.   # (mV)
...
if benchmark == "COBA":
    ext_stim = p.Population(20, p.SpikeSourcePoisson,
                            {'rate': rate, 'duration': stim_dur},
                            label="expoisson")
    rconn = 0.01
    ext_conn = p.FixedProbabilityConnector(rconn, weights=0.1)
    p.external_devices.activate_live_output_for(ext_stim)
    ext_stim.record()

print "%s Initialising membrane potential to random values..." % node_id
rng = NumpyRNG(seed=rngseed, parallel_safe=parallel_safe)
uniformDistr = RandomDistribution('uniform', [v_reset,
                                              v_thresh], rng=rng)
exc_cells.initialize('v', uniformDistr)
inh_cells.initialize('v', uniformDistr)

print "%s Connecting populations..." % node_id
exc_conn = p.FixedProbabilityConnector(pconn, weights=w_exc, delays=delay)
inh_conn = p.FixedProbabilityConnector(pconn, weights=w_inh, delays=delay)
...
#    connections['e2sink'] = Projection(exc_cells, sink,
# AllToAllConnector(weights=1, delays=1), target='excitatory')
#    connections['i2sink'] = Projection(inh_cells, sink,
# AllToAllConnector(weights=1, delays=1), target='excitatory')

if (benchmark == "COBA"):
    connections['ext2e'] = p.Projection(ext_stim, exc_cells, ext_conn,
                                        target='excitatory')
    connections['ext2i'] = p.Projection(ext_stim, inh_cells, ext_conn,
                                        target='excitatory')

# === Setup recording =========================================================
print "%s Setting up recording..." % node_id
exc_cells.record()
p.external_devices.activate_live_output_for(exc_cells)
# inh_cells.record()
# exc_cells.record_v()
# inh_cells.record_v()
...
# connections[prj].saveConnections('Results/VAbenchmark_%s_%s_%s_np%d.conn'
#  % (benchmark, prj, simulator_name, np))
...
# exc_cells.printSpikes("Results/VAbenchmark_%s_exc_%s.ras" %
# (benchmark, simulator_name))
# inh_cells.printSpikes("Results/VAbenchmark_%s_inh_%s.ras" %
# (benchmark, simulator_name))

exc_spikes = exc_cells.getSpikes()
pylab.scatter([time for (time, neuron_id) in exc_spikes],
              [neuron_id for (time, neuron_id) in exc_spikes], s=2,
              color='blue')
pylab.show()


# exc_cells.print_v("Results/VAbenchmark_%s_exc_%s.v" %
# (benchmark, simulator_name))
