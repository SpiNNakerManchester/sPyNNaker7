...
def do_run(plot):
    ...
    for group_index in range(n_groups-1):
        p.Projection(exc_pops[group_index % n_groups],
                     ...
                     p.FixedNumberPostConnector(90, weights=weight_exc,
                                                delays=10.),
    ...
    p.Projection(exc_pops[1], exc_pops[1],
                 p.FixedNumberPostConnector(90, allow_self_connections=False,
    ...
    p.Projection(pop_stim, inh_pops[0],
                 p.FixedNumberPostConnector(50, weights=weight_exc,
                                            delays=20.),
                 target='excitatory', rng=rng)
...
class FixedNumberPostConnectorTest(BaseTestCase):
    def test_run(self):
        ...
        self.assertGreater(2000, len(spklist_exc))
        self.assertGreater(1700, len(spklist_inh))
