...
def do_run(plot):
    ...
    p.set_number_of_neurons_per_core("IF_cond_exp", 100)
    ...
    for group_index in range(n_groups):
        ...
        p.Projection(inh_pop, exc_pop,
                     p.AllToAllConnector(weights=weight_inh, delays=8.),
    ...
    pop_stim.tset('spike_times', all_spiketimes)
    ...
    for group_index in range(n_groups-1):
        p.Projection(exc_pops[group_index % n_groups],
                     ...
                     p.FixedNumberPostConnector(80, weights=weight_exc,
                                                delays=10.),
                     target='excitatory', rng=rng)
        p.Projection(exc_pops[group_index % n_groups],
                     inh_pops[(group_index+1) % n_groups],
                     p.FixedNumberPostConnector(20, weights=weight_exc,
                                                delays=10.),
                     target='excitatory', rng=rng)

    # Make another projection for testing that connects to itself
    p.Projection(exc_pops[1], exc_pops[1],
                 p.FixedNumberPostConnector(60, allow_self_connections=False,
    ...
    p.Projection(pop_stim, inh_pops[0],
                 p.FixedNumberPostConnector(15, weights=weight_exc,
                                            delays=20.),
                 target='excitatory' ,rng=rng)
    ...
    for group in range(n_groups):
        EXC_spikes = exc_pops[group].getSpikes()
        INH_spikes = inh_pops[group].getSpikes()
...
class FixedNumberPostConnectorTest(BaseTestCase):
    def test_run(self):
        ...
        self.assertGreater(1900, len(spklist_exc))
        self.assertGreater(500, len(spklist_inh))
        self.assertLess(1200, len(spklist_exc))
        self.assertLess(300, len(spklist_inh))
