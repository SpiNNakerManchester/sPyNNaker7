...
def do_run(plot):
    ...
    for group_index in range(n_groups-1):
        p.Projection(exc_pops[group_index % n_groups],
                     ...
                     p.FixedNumberPreConnector(90, weights=weight_exc,
                                               delays=10.),
    ...
    p.Projection(exc_pops[1], exc_pops[1],
                 p.FixedNumberPreConnector(90, allow_self_connections=False,
    ...
    p.Projection(pop_stim, inh_pops[0],
                 p.FixedNumberPreConnector(50, weights=weight_exc, delays=20.),
                 target='excitatory', rng=rng)
...
class FixedNumberPreConnectorTest(BaseTestCase):
    def test_run(self):
        ...
        self.assertGreater(2200, len(spklist_exc))
        self.assertGreater(1900, len(spklist_inh))
        self.assertLess(1000, len(spklist_exc))
        self.assertLess(800, len(spklist_inh))
